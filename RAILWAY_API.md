...
### 0. Автоматический Deploy + Мониторинг: `scripts/deploy_and_monitor.sh` ⭐

**Рекомендуется для всех git push операций!**

Автоматизирует весь процесс деплоя и проверки:

```bash
# Автоматический deploy с проверкой
./scripts/deploy_and_monitor.sh "Описание изменений"

# Скрипт выполняет:
# 1. git add . && git commit && git push
# 2. Ждет 90 секунд (Railway запускает deployment)
# 3. Проверяет статус deployment
# 4. Предлагает запустить real-time мониторинг
```

**Преимущества:**
- ✅ Автоматическая проверка деплоя
- ✅ Не забудешь проверить статус
- ✅ Интерактивный мониторинг
- ✅ Единая команда для всего workflow
