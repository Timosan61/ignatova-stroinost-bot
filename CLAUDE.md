...
### 1. Проверка Railway Deployments
**После КАЖДОГО `git push origin main` ОБЯЗАТЕЛЬНО:**
1. Подождать 90 секунд (Railway запускает автодеплой)
2. Проверить статус deployment:
   ```bash
   sleep 90
   python3 scripts/railway_monitor.py info
   ```
3. Использовать документацию в **`RAILWAY_API.md`** для всех операций с Railway API
4. При ошибках деплоя - немедленно сообщить пользователю с деталями

### 2. Обязательное обновление GitHub
**При ЛЮБЫХ изменениях в коде - СРАЗУ обновлять GitHub** (см. секцию "Automatic GitHub Updates" ниже)
...
```bash
# Автоматический deploy + мониторинг
./scripts/deploy_and_monitor.sh "Описание изменений"

# Скрипт автоматически:
# 1. Делает git add/commit/push
# 2. Ждет 90 секунд
# 3. Проверяет статус deployment
# 4. Предлагает запустить мониторинг
```
...
# КРИТИЧЕСКИ ВАЖНО: Подождать 90 секунд и проверить деплой!
sleep 90
python3 scripts/railway_monitor.py info
```
...
1. **Для проверки логов Railway ВСЕГДА используй документацию в `RAILWAY_API.md`**
2. **После каждого `git push origin main` ОБЯЗАТЕЛЬНО через 1 минуту 30 секунд проверяй логи деплоя:**
   ```bash
   # Подожди 90 секунд после push
   sleep 90

   # Проверь статус последнего deployment
   python3 scripts/railway_monitor.py info
...
3. При диагностике проблем с деплойментом - ВСЕГДА использовать Railway API для автоматической проверки логов и переменных окружения.
